 ...
 */
class ParserRuleContext extends _RuleContext.default {
  ...
  addTokenNode(token) {
    const node = new _TerminalNodeImpl.default(token);
    this.addChild(node);
    node.parentCtx = this;
    return node;
  }
  addErrorNode(badToken) {
    const node = new _ErrorNodeImpl.default(badToken);
    this.addChild(node);
    node.parentCtx = this;
    return node;
  }
...
_RuleContext.default.EMPTY = new ParserRuleContext();