...
class LexerATNSimulator extends _ATNSimulator.default {
  /**
  ...
  execATN(input, ds0) {
    ...
    for (;;) {
      ...
      if (target === _ATNSimulator.default.ERROR) {
        break;
      }
  ...
  computeTargetState(input, s, t) {
    const reach = new _OrderedATNConfigSet.default();
    ...
    if (reach.items.length === 0) {
      ...
      if (!reach.hasSemanticContext) {
        ...
        this.addDFAEdge(s, t, _ATNSimulator.default.ERROR);
      }
  ...
  getReachableConfigSet(input, closure, reach, t) {
    ...
    let skipAlt = _ATN.default.INVALID_ALT_NUMBER;
    for (let i = 0; i < closure.items.length; i++) {
      ...
      for (let j = 0; j < cfg.state.transitions.length; j++) {
        ...
        if (target !== null) {
          ...
          const config = new _LexerATNConfig.default({
            state: target,
  ...
  computeStartState(input, p) {
    const initialContext = _PredictionContext.default.EMPTY;
    const configs = new _OrderedATNConfigSet.default();
    for (let i = 0; i < p.transitions.length; i++) {
      const target = p.transitions[i].target;
      const cfg = new _LexerATNConfig.default({
        state: target,
        alt: i + 1,
  ...
  getEpsilonTarget(input, config, trans, configs, speculative, treatEofAsEpsilon) {
    ...
    if (trans.serializationType === _Transition.default.RULE) {
      const newContext = _SingletonPredictionContext.default.create(config.context, trans.followState.stateNumber);
      cfg = new _LexerATNConfig.default({
        state: trans.target,
        context: newContext
      }, config);
      ...
      if (config.context === null || config.context.hasEmptyPath()) {
        ...
        // the split operation.
        const lexerActionExecutor = _LexerActionExecutor.default.append(config.lexerActionExecutor, this.atn.lexerActions[trans.actionIndex]);
        cfg = new _LexerATNConfig.default({
          state: trans.target,
          lexerActionExecutor: lexerActionExecutor
        }, config);
   ...
   */
  addDFAState(configs) {
    const proposed = new _DFAState.default(null, configs);
