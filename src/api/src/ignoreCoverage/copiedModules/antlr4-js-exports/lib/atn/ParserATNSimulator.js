...
var _ATNSimulator = _interopRequireDefault(require("./ATNSimulator.js"));
 ...
 * the input.</p>
 */
class ParserATNSimulator extends _ATNSimulator.default {
  constructor(parser, atn, decisionToDFA, sharedContextCache) {
    super(atn, sharedContextCache);
    this.parser = parser;
    this.decisionToDFA = decisionToDFA;
    // SLL, LL, or LL + exact ambig detection?//
    this.predictionMode = _PredictionMode.default.LL;
  ...
  adaptivePredict(input, decision, outerContext) {
    ...
    try {
      ...
      if (s0 === null) {
        ...
        const fullCtx = false;
        let s0_closure = this.computeStartState(dfa.atnStartState, _RuleContext.default.EMPTY, fullCtx);
        if (dfa.precedenceDfa) {
  ...
  execATN(dfa, s0, input, startIndex, outerContext) {
    ...
    for (;;) {
      ...
      if (D === _ATNSimulator.default.ERROR) {
  ...
  computeTargetState(dfa, previousD, t) {
    ...
    if (reach === null) {
      this.addDFAEdge(dfa, previousD, t, _ATNSimulator.default.ERROR);
      return _ATNSimulator.default.ERROR;
    }
    // create new target state; we'll add to DFA after it's complete
    let D = new _DFAState.default(null, reach);
    const predictedAlt = this.getUniqueAlt(reach);
    if (this.debug) {
      const altSubSets = _PredictionMode.default.getConflictingAltSubsets(reach);
      console.log("SLL altSubSets=" + (0, _arrayToString.default)(altSubSets) + /*", previous=" + previousD.configs + */
  ...
  computeReachSet(closure, t, fullCtx) {
    ...
    const intermediate = new _ATNConfigSet.default(fullCtx);
    ...
    for (let i = 0; i < closure.items.length; i++) {
      ...
      for (let j = 0; j < c.state.transitions.length; j++) {
        ...
        if (target !== null) {
          const cfg = new _ATNConfig.default({
            state: target
          }, c);
    ...
    if (reach === null) {
      ...
      const closureBusy = new _HashSet.default();
      const treatEofAsEpsilon = t === _Token.default.EOF;
  ...
  removeAllConfigsNotInRuleStopState(configs, lookToEndOfRule) {
    ...
    const result = new _ATNConfigSet.default(configs.fullCtx);
  ...
  computeStartState(p, ctx, fullCtx) {
    ...
    const initialContext = (0, _PredictionContextUtils.predictionContextFromRuleContext)(this.atn, ctx);
    const configs = new _ATNConfigSet.default(fullCtx);
    if (this.trace_atn_sim) {
  ...
  applyPrecedenceFilter(configs) {
    ...
    const configSet = new _ATNConfigSet.default(configs.fullCtx);
    for (let i = 0; i < configs.items.length; i++) {
  ...
  getPredsForAmbigAlts(ambigAlts, configs, nalts) {
    ...
    for (let i = 1; i < nalts + 1; i++) {
      ...
      if (pred === null) {
        altToPred[i] = _SemanticContext.default.NONE;
  ...
  splitAccordingToSemanticValidity(configs, outerContext) {
    const succeeded = new _ATNConfigSet.default(configs.fullCtx);
    const failed = new _ATNConfigSet.default(configs.fullCtx);
    for (let i = 0; i < configs.items.length; i++) {
      const c = configs.items[i];
  ...
  evalSemanticContext(predPredictions, outerContext, complete) {
    const predictions = new _BitSet.default();
    ...
    if (config.state instanceof _RuleStopState.default) {
      ...
      if (!config.context.isEmpty()) {
        for (let i = 0; i < config.context.length; i++) {
          ...
          const c = new _ATNConfig.default(parms, null);
  ...
  closure_(config, configs, closureBusy, collectPredicates, fullCtx, depth, treatEofAsEpsilon) {
    ...
    for (let i = 0; i < p.transitions.length; i++) {
      ...
      const continueCollecting = collectPredicates && !(t instanceof _ActionTransition.default);
  ...
  canDropLoopEntryEdgeInLeftRecursiveRule(config) {
    ...
    if (p.stateType !== _ATNState.default.STAR_LOOP_ENTRY || !p.isPrecedenceDecision || config.context.isEmpty() || config.context.hasEmptyPath()) return false;
    ...
    for (let i = 0; i < numCtxs; i++) {
      ...
      if (returnState.stateType === _ATNState.default.BLOCK_END && returnStateTarget === p) continue;
      ...
      if (returnStateTarget.stateType === _ATNState.default.BLOCK_END && returnStateTarget.transitions.length === 1 && returnStateTarget.transitions[0].isEpsilon && returnStateTarget.transitions[0].target === p) continue;
  ...
  precedenceTransition(config, pt, collectPredicates, inContext, fullCtx) {
    ...
    if (collectPredicates && inContext) {
      if (fullCtx) {
        ...
        const newSemCtx = _SemanticContext.default.andContext(config.semanticContext, pt.getPredicate());
        c = new _ATNConfig.default({
          state: pt.target,
  ...
  ruleTransition(config, t) {
    ...
    const newContext = _SingletonPredictionContext.default.create(config.context, returnState.stateNumber);
    return new _ATNConfig.default({
      state: t.target,
      context: newContext
    }, config);
  ...
  getConflictingAlts(configs) {
    const altsets = _PredictionMode.default.getConflictingAltSubsets(configs);
    ...
    for (let i = 0; i < decs.length; i++) {
      ...
      if (c.state.transitions.length > 0) {
        ...
        if (t instanceof _AtomTransition.default) {
          ...
          const neg = t instanceof _NotSetTransition.default;
          trans = (neg ? "~" : "") + "Set " + t.set;
        }
      }
  ...
  getUniqueAlt(configs) {
    let alt = _ATN.default.INVALID_ALT_NUMBER;
    for (let i = 0; i < configs.items.length; i++) {
      const c = configs.items[i];
      if (alt === _ATN.default.INVALID_ALT_NUMBER) {
  ...
  reportAttemptingFullContext(dfa, conflictingAlts, configs, startIndex, stopIndex) {
    if (this.debug || this.retry_debug) {
      const interval = new _Interval.default(startIndex, stopIndex + 1);
